...
public class OSSUtil {
    ...
    private final static String BUCKET_NAME = "gaorunze";
    ...
    public boolean buildTmpUrl(SystemFile systemFile){
        if(systemFile == null){
            return false;
        }
        String objectName = systemFile.getPath();
        if(BaseTypeUtil.hasText(objectName)){
            String tmpUrl = getTmpUrl(objectName);
            if(BaseTypeUtil.hasText(tmpUrl)){
                systemFile.setUrl(tmpUrl);
                return true;
            }
            return false;
        }
