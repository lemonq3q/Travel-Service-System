...
@Component
public class ScenicRatingScheduler {
    ...
    public void refreshScenicRatings() {
        List<Scenic> scenics = scenicMapper.selectList(new LambdaQueryWrapper<Scenic>()
                .eq(Scenic::getIsDelete, 0));
        ...
        try {
            ResponseResult<Map<String, Object>> res = commentStatsFeignClient.scenicAvgRatings(null);
        ...
        for (Scenic s : scenics) {
            ...
            BigDecimal rating = avgMap.getOrDefault(s.getId(), new BigDecimal("5.0"));
            Scenic upd = new Scenic();
            upd.setId(s.getId());
            upd.setRating(rating);
            upd.setUpdateTime(now);
            upd.setUpdateBy(0L);
            scenicMapper.updateById(upd);
        }
    }
}
